...
WARMENTR       JMP  MAINLOOP   ; warm entry point
               JMP  SRCHTBL    ; command table search entry point
                               ;
...
PROMPT                         ; prompt string (maximum length, 8 bytes)
               ...
               DS   $06        ; filler
                               ;
               DFB  <MAINCT    ; address (lo hi) of main command table
               DFB  >MAINCT    ;
               DFB  <EDMAINCT  ; address (lo hi) of end of main command table
               DFB  >EDMAINCT  ;
                               ;
COMTBL         DS   $02        ; address of start of command table for SRCHTBL
EDCOMTBL       DS   $02        ; address of end of command table for SRCHTBL
                               ;
INITADDR                       ; address of initial command file
               DFB  <COMINIT,>COMINIT
                               ;
PTHADDR                        ; address of default pathname
               DFB  <PTH,>PTH
                               ;
INPADDR                        ; address of current input pathname
               DFB  <CURINP,>CURINP
                               ;
               ASC             "Command interpreter system"
               ASC             "Christopher A. Mosher"



MAINCT                         ; main table of commands
                               ;   Preceding each command string is
                               ;   the address of its handler,
                               ;   and the number of characters required
                               ;   for a match.
                               ;   The string is terminated with a 0.

                               ; (for a blank line)
               DFB  <MAINLOOP,>MAINLOOP
               DFB  $00
               DFB  $00
               ...
               DFB  $00
                               ...
                               ; print the directory of a disk
               DFB  <DIRECT,>DIRECT
               DFB  $02
               ASC  "directory"
               DFB  $00

EDMAINCT                       ; end of main command table



PTH                            ; current default pathname
                               ; (maximum length, $FF bytes)
                               ; (terminated with 0)
                               ; (initially empty)
               ASC  "/"
               DFB  $00
...
INITSTK        LDX  #$FF       ; initialize stack
               ...
               LDA  #<RESET    ; initialize reset vector
               STA  SOFTEV
               LDA  #>RESET
               STA  SOFTEV+1
               EOR  #$A5
...
PROCCOM                        ; process the command
                               ...
                               ;                   with $8D or $0D
                               ; output: executed command or 'unknown' message
...
SEEDFT                         ; check current default pathname
                               ; input : string following JSR SEEDFT
                               ;         INPADDR: address of current
                               ;                  input pathname
                               ; output: C: set  : equal
                               ;            clear: not equal
               PLA
